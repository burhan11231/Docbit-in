...
import { Plus, Folder, Search, Filter, Star, HardDrive, Edit, Trash2, Archive, Link as LinkIcon, RotateCcw, Copy, Tag, Image as ImageIcon } from 'lucide-react';
...
export function ProjectList() {
  ...
  const fetchData = async () => {
    try {
      ...
      // If we have workspaces, fetch projects for the first one for now
      if (workspacesData.workspaces && workspacesData.workspaces.length > 0) {
        setSelectedWorkspaceId(workspacesData.workspaces[0].id);
        const projectsData = await api.projects.list(workspacesData.workspaces[0].id);
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm();
      const projectsData = await api.projects.list(selectedWorkspaceId);
      setProjects(projectsData.projects || []);
    } catch (error) {
  ...
  const toggleArchive = async (project: any) => {
    try {
      ...
      await api.projects.update(project.id, { is_deleted: newStatus });
      const projectsData = await api.projects.list(selectedWorkspaceId);
      setProjects(projectsData.projects || []);
    } catch (error) {
  ...
  const togglePin = async (e: React.MouseEvent, project: any) => {
    e.stopPropagation();
    try {
  ...
  const filteredProjects = projects
    .filter(p => {
      ...
      const matchesStatus = filterStatus === 'archived' ? p.is_deleted === true : (p.is_deleted === false || p.is_deleted === null);
          ...
          <AnimatePresence>
            {isFilterDropdownOpen && (
              ...
              >
                ...
                <button className={`w-full text-left px-5 py-2.5 text-sm hover:bg-indigo-50 transition-colors ${filterStatus === 'active' ? 'font-semibold bg-indigo-50/50 text-indigo-700' : 'text-slate-700'}`} onClick={() => { setFilterStatus('active'); setIsFilterDropdownOpen(false); }}>Active</button>
                <button className={`w-full text-left px-5 py-2.5 text-sm hover:bg-indigo-50 transition-colors ${filterStatus === 'archived' ? 'font-semibold bg-indigo-50/50 text-indigo-700' : 'text-slate-700'}`} onClick={() => { setFilterStatus('archived'); setIsFilterDropdownOpen(false); }}>Archived</button>
